...
const AttributeForm = ({
  ...
  const addValidation = (value) => {
    if (!attribute.validations.includes(value)) {
      setAttribute({
        ...attribute,
        validations: [...attribute.validations, value],
      });
    }
  };
  ...
  const deleteValidation = (value) => {
    setAttribute({
      ...attribute,
      validations: attribute.validations.filter((v) => v !== value),
    });
  };
  ...
  const deleteAttribute = () => {
    setEntity({
      ...entity,
      attributes: entity.attributes.filter((a) => a.uuid !== uuid),
    });
  };
  ...
  return (
    ...
    >
      <div className="form-header">
        ...
        />
        <IconButton aria-label="delete" onClick={deleteAttribute}>
          <DeleteIcon />
        </IconButton>
      </div>
      <InputLabel id={`attribute-type-${attribute.uuid}`}>
        Tipo del atributo
      </InputLabel>
      <Select
        color="success"
      ...
      {!!attribute.type.includes("CUSTOM") && (
        <>
          <InputLabel id={`attribute-type-custom-${attribute.uuid}`}>
            Tipo CUSTOM del atributo
          </InputLabel>
          <Select
            color="success"
            ...
            onChange={({ target: { value } }) =>
              changeAttributeCustomType(value)
            }
          >
            {entities
              .filter((e) => e.uuid !== entity.uuid)
              .map((e) => (
                <MenuItem value={e.entityName}>{e.entityName}</MenuItem>
              ))}
          </Select>
        </>
      )}
      <InputLabel id={`attribute-validations-${attribute.uuid}`}>
        Validaciones
      </InputLabel>
      <Select
        color="success"
      ...
      >
        ...
        {attribute.type === "STRING" && (
          <MenuItem value={"NOT_BLANK"}>String no vacio</MenuItem>
        )}
        {attribute.type === "CUSTOM_LIST" && (
          <MenuItem value={"NOT_EMPTY"}>Array no vacio</MenuItem>
        )}
      </Select>
      {!!attribute.validations.length && (
        <Typography>Validaciones seleccionadas:</Typography>
      )}
      {attribute.validations.map((v) => (
        <Chip key={v} label={v} onDelete={() => deleteValidation(v)} />
      ))}
    </Card>
  );
};
