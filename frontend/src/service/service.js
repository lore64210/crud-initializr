...
const formatStringToCamelCase = (val) => {
    ...
    let newVal = words
        .map((w, indx) => indx !== 0 ? w[0].toUpperCase() + w.substring(1) : w)
    return newVal.join('');
}
...
const createRequestBody = (data) => {
    const entities = data.entities.map(e => {
        ...
        e.attributes.forEach(a => {
            if (a.name) {
                attributes = {...attributes, [formatStringToCamelCase(a.name)]: {...a}};
            }
        })
...
const service = {
    createProject: async (data) => {
        const response = await fetch(`/api/domain`, {method: 'POST', body: JSON.stringify(createRequestBody(data)), headers: { 'Content-Type': 'application/json'}});
        if (!response.ok) {
